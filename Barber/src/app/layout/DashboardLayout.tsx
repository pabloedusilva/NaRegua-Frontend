  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-bg to-bg-soft">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50">
        <div className="relative bg-black/95 backdrop-blur-xl border-t border-border/50">
          ...
          <div className="overflow-x-auto scrollbar-hide">
            <div className="flex items-center justify-around px-2 py-3 pb-safe min-w-max">
              {navItems.slice(0, 5).map((item) => (
                <NavLink
                  key={item.path}
                  to={item.path}
                  className="flex-1 min-w-[64px] max-w-[80px]"
                >
                  {({ isActive }) => (
                    <div className="relative">
                      {isActive && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-12 h-1 bg-gold rounded-full"></div>
                      )}
                      <div className="flex flex-col items-center gap-1.5 py-2">
                        ...
                        <span className={`text-[10px] font-medium tracking-tight transition-colors duration-200 text-center ${
      ...
      <div className="lg:ml-64">
        {/* Top Bar */}
        <header className="sticky top-0 z-30 bg-[#0a0a0a]/95 backdrop-blur-lg border-b border-border/50">
          <div className="flex items-center justify-between px-3 md:px-6 py-3 md:py-4">
            ...
            <div className="flex items-center gap-2 lg:hidden">
              ...
              />
              <div>
                <h2 className="font-display text-gold text-base md:text-lg">Régua Máxima</h2>
              </div>
            </div>

            <div className="flex items-center gap-3 md:gap-4 ml-auto">
              <div className="text-right hidden sm:block">
                            <p className="text-sm font-semibold text-text truncate max-w-[150px] md:max-w-none">{(typeof window !== 'undefined' && JSON.parse(localStorage.getItem('barbershop_profile') || '{}')?.name) || 'Régua Máxima'}</p>
              ...
              <button
                ...
                className="relative w-11 h-11 md:w-12 md:h-12 rounded-full overflow-hidden shadow-lg shadow-gold/20 hover:scale-105 active:scale-95 transition-transform border-2 border-gold/30"
              >
                <img
              ...
              {showLogoutMenu && (
                <>
                  ...
                  />
                  <div className="absolute top-full right-3 md:right-6 mt-2 w-56 bg-[#0a0a0a] border border-gold/20 rounded-xl shadow-2xl shadow-gold/10 overflow-hidden z-50 animate-fade-in">
                    ...
                    <button
                      onClick={handleLogout}
                      className="w-full flex items-center gap-3 px-4 py-3 text-red-400 hover:bg-red-500/10 active:bg-red-500/20 transition-all"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                      </svg>
        ...
        {/* Page Content */}
        <main className="p-3 sm:p-4 md:p-6 lg:p-8 pb-24 lg:pb-20">
          <Outlet />
        </main>

        {/* Footer - esconde em mobile PWA */}
